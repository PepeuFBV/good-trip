...
├── .github/
│   ├── workflows/
│   │   ├── release.yml          # semantic-release CI (runs on push to main)
│   │   └── ci.yml               # PR linting, shellcheck, structure validation
│   └── PULL_REQUEST_TEMPLATE.md
├── bin/
│   └── good-trip                # CLI entrypoint (symlinked to ~/.local/bin/)
├── bootstrap/                   # Idempotent install scripts per component
│   ├── install-zsh.sh
│   ├── install-ohmyzsh.sh
...
│   ├── install-atuin.sh
│   └── install-git-gpg.sh
├── config/                      # Managed dotfiles (symlinked into $HOME)
│   ├── zsh/
│   │   └── .zshrc               # → ~/.zshrc
│   ├── git/
│   │   └── config               # → ~/.gitconfig
│   └── aliases/
│       ├── docker.aliases       # → ~/.shell/aliases/ (whole dir)
│       └── node.aliases
├── scripts/
│   ├── symlinks.sh              # Create / repair all managed symlinks
│   ├── update.sh                # Full update logic
│   └── check-version.sh        # Lightweight version check (called by .zshrc)
├── install.sh                   # Main entry point (orchestrates everything)
├── update.sh                    # Root convenience delegator → scripts/update.sh
├── version.txt                  # Current version (managed by semantic-release)
├── CHANGELOG.md                 # Auto-generated by semantic-release
├── .releaserc.json              # semantic-release configuration
├── .commitlintrc.json           # Conventional Commits rule set
└── README.md
```
